...
const Sidebar = () => {
  ...
  return (
    <div
      className={` ${isOpen ? "md:mr-[250px]  duration-100" : "md:mr-[75px]"}`}
    >
      <button
        className={`text-right p-3 fixed md:hidden right-2 top-2 ${!isMbOpen ? "" : "rotate-90"}`}
        onClick={() => setIsMbOpen(!isMbOpen)}
      >
        {" "}
        |||{" "}
      </button>
      ...
      >
        ...
        <nav className="2xl:mt-4 mt-2 flex flex-col justify-between md:h-[80vh] 2xl:h-[85vh]">
          ...
          <div className="mx-4 flex flex-col items-center">
            <Avatar
              className={` ${isOpen ? "md:size-20" : "md:size-8"} size-8 `}
            ...
            <Button
              className="md:flex hidden float-end w-full"
              color="danger"
              onClick={() => logout()}
            >
              {" "}
    ...
    >
      <Link passHref href={href}>
        <p className="flex items-center gap-3 w-full text-white">
          <span
            className={` ${isOpen ? "text-md" : "text-2xl mx-auto text-center"}`}
          >
            {icon}
          </span>
